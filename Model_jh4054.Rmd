...
fit_forward = lm(target_death_rate ~ pct_bach_deg25_over + incidence_rate + pct_public_coverage_alone + 
    ...
    birth_rate + pct_married_households + percent_married + pct_unemployed16_over, data = cancer_reg_tidy)
...
target_death_rate ~ incidence_rate + median_age_male + percent_married + pct_hs18_24 + pct_bach_deg25_over + pct_unemployed16_over + pct_public_coverage_alone + pct_white + pct_other_race + pct_married_households + birth_rate
...
# backward fit
fit_back = lm(target_death_rate ~ incidence_rate + binned_inc + median_age_male + 
    percent_married + pct_hs18_24 + pct_bach_deg25_over + pct_unemployed16_over + 
...
target_death_rate ~ pct_bach_deg25_over + incidence_rate + pct_public_coverage_alone + 
    pct_other_race + pct_white + pct_hs18_24 + median_age_male + 
    birth_rate + pct_married_households + percent_married + pct_unemployed16_over

```{r}
# stepwise regression, both directions
step(null, scope = list(upper=full), data = cancer_reg_tidy, direction="both")

fit_both_dir = lm(target_death_rate ~ pct_bach_deg25_over + incidence_rate + binned_inc + pct_married_households + pct_hs18_24 + pct_other_race + median_age_male + 
    birth_rate + pct_public_coverage_alone + percent_married + 
    pct_unemployed16_over + pct_white, data = cancer_reg_tidy)
...
* For Mallow's Cp-statistic, the Cp Statistic should be less than or equal to the # of parameters. Optimizing for lowest Cp Statistic, we select 15 parameters.
* For R^2(adj) criteria, we optimize for highest R^2(adj), while accounting for parsimony. Selecting ~10 parameters optimizes both the R^2(adj) criterion and overarching goal for parsimony. Not much improvement over 10 parameters. 
...
=======
  subsets <- regsubsets(formula(model), nvmax = 21, model.frame(model), ...)
>>>>>>> e2af2b977559a99e902acd5b71bbe185d187ed70
...
=======
cp_r2_df = round(best(fit_multi, nbest = 1), digits = 4)

cp_r2_df
>>>>>>> e2af2b977559a99e902acd5b71bbe185d187ed70
```

The adj R2 looks pretty good for ~10 predictor models (0.48-0.49), but the Cp Criteria is way too high. 
```{r}

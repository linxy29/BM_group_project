...
12-Predictor Model(adj. R2: 0.5096): target_death_rate ~ pct_bach_deg25_over + incidence_rate + binned_inc + pct_married_households + pct_hs18_24 + pct_other_race + median_age_male + 
    birth_rate + pct_public_coverage_alone + percent_married + 
    pct_unemployed16_over + pct_white
...
# forward fit
fit_forward = lm(target_death_rate ~ pct_bach_deg25_over + incidence_rate + binned_inc + 
    pct_married_households + pct_hs18_24 + pct_other_race + median_age_male + 
    birth_rate + pct_public_coverage_alone + percent_married + 
...
12-predictor Model (Adjusted R-squared:  0.5096) :target_death_rate ~ incidence_rate + binned_inc + median_age_male + percent_married + pct_hs18_24 + pct_bach_deg25_over + pct_unemployed16_over + 
...
Automatic Search Procedures using forward & backward selection, stepwise regression generated the same "best model" with 12-parameters, using the AIC criterion. The model has Adjusted R-squared:  0.5096. 
...
* We should have a 10 parameter and 15 parameter model. 

```{r}
criterion = leaps::regsubsets(target_death_rate ~ ., data = cancer_reg_tidy, nvmax = 20)
...
# Cp Statistic Plot 
plot(x = 1:20, y = crit_sum$cp, xlab = "No of parameters", ylab = "Cp Statistic")
# abline - adds straight lines to a plot
abline(0,1)

# Adjusted R2 Plot
plot(x = 1:20, y = crit_sum$adjr2, xlab = "No of parameters", ylab = "Adj R2")
```
